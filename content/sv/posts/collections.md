...
Samlingskonfigurationen lagras i `content/_data/content-types.json`. Du kan visa och redigera den här filen direkt, eller använda CLI-verktygen som beskrivs nedan.
...
Efter att du har [lagt till ett nytt språk](/documentation/adding-additional-languages/), kanske du vill redigera dina samlingar för att lägga till anpassade slug för varje samlingstyp.
...
När du tar bort en samling raderas den från `content-types.json` och alla innehållsfiler på alla språk tas bort. CLI visar endast icke-skyddade samlingar i urvalslistan, eftersom skyddade samlingar inte kan tas bort. Du kommer att bli ombedd att bekräfta innan några filer raderas.

``` bash
npm run cli collection remove
```

## Redigera content-types.json

All samlingskonfiguration lagras i `content/_data/content-types.json`. Du kan redigera den här filen direkt om du föredrar det. Så här ser filen ut:
...
Efter att ha redigerat `content-types.json` manuellt måste du skapa motsvarande mappar under varje språkkatalog och lägga till en `{collectionname}.11tydata.js`-fil för varje samling. Mallen för denna fil finns i `.cli/templates/collection.11tydata.js` och kan kopieras till varje ny samlingsmapp. Du kan också köra `npm run cli sync-collections` för att återskapa alla samlingsmallfiler på en gång.
